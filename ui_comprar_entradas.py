...
class ComprarEntradasUI(ctk.CTk):
    def __init__(self, usuario_logueado: Usuario):
        ...
        self.color_principal = "#3E8914"
        self.color_secundario = "#96E072"

        self.title("🎟️ Compra de Entradas - EcoHarmony Park")
        self.geometry("600x600")

        # Logo del parque
        logo_img = ctk.CTkImage(light_image=Image.open(r"C:\Users\Mili\IngenieriaSistemas\ingenieriaCalidadSoftware\2025\ISW-TEST\Ecoharmony park - logo.png"), size=(100, 100))
        ctk.CTkLabel(self, image=logo_img, text="").pack(pady=(10, 0))

        ctk.CTkLabel(self, text=f"Bienvenido, {self.usuario_logueado.email}", font=self.fuente_montserrat).pack(pady=10)

        ctk.CTkLabel(self, text="Seleccione la fecha de visita:", font=self.fuente_montserrat).pack(pady=(10, 5))
        ...
        self.om_anio = ctk.CTkOptionMenu(self.fecha_frame, variable=self.anio_var, values=[
                                         str(today.year), str(today.year + 1)], command=self.actualizar_dias, font=self.fuente_montserrat, fg_color=self.color_principal, button_color=self.color_secundario)
        self.om_mes = ctk.CTkOptionMenu(self.fecha_frame, variable=self.mes_var, values=[
                                        str(m) for m in range(1, 13)], command=self.actualizar_dias, font=self.fuente_montserrat, fg_color=self.color_principal, button_color=self.color_secundario)
        self.om_dia = ctk.CTkOptionMenu(
            self.fecha_frame, variable=self.dia_var, command=self.actualizar_dias, font=self.fuente_montserrat, fg_color=self.color_principal, button_color=self.color_secundario)
        ...
        self.label_feedback_fecha = ctk.CTkLabel(
            self, text="", font=self.fuente_montserrat)
        self.label_feedback_fecha.pack(pady=(5, 10))

        ctk.CTkLabel(self, text="Cantidad de entradas:", font=self.fuente_montserrat).pack(pady=(15, 5))
        ...
        self.btn_menos = ctk.CTkButton(
            self.cantidad_frame, text="-", width=40, command=lambda: self.ajustar_cantidad(-1), fg_color=self.color_principal, font=self.fuente_montserrat)
        ...
        ctk.CTkLabel(self, text="Tipo de pase:", font=self.fuente_montserrat).pack(pady=(15, 5))
        self.tipo_pase = ctk.CTkOptionMenu(self, values=["regular", "VIP"], font=self.fuente_montserrat, fg_color=self.color_principal, button_color=self.color_secundario)
        self.tipo_pase.set("regular")
        self.tipo_pase.pack()

        ctk.CTkLabel(self, text="Forma de pago:", font=self.fuente_montserrat).pack(pady=5)
        self.forma_pago = ctk.CTkOptionMenu(
            self, values=["efectivo", "tarjeta"], font=self.fuente_montserrat, fg_color=self.color_principal, button_color=self.color_secundario)
        self.forma_pago.set("efectivo")
        self.forma_pago.pack()

        self.btn_comprar = ctk.CTkButton(
            self, text="Comprar entradas", command=self.realizar_compra, fg_color=self.color_principal, font=self.fuente_montserrat)
